                    ...
                    steps {
                        dir('workspace-dev') {
                            ...
                            dir('angular-17-client') {
                                bat 'npm install'
                                bat 'npx ng build --configuration dev --base-href /dev/'
                            }
                            script {
                ...
                stage('Deploy TEST environment') {
                    ...
                    steps {
                        dir('workspace-test') {
                            ...
                            dir('angular-17-client') {
                                bat 'npm install'
                                bat 'npx ng build --configuration test --base-href /test/'
                            }
                            script {
                ...
                stage('Deploy STAGING environment') {
                    ...
                    steps {
                        dir('workspace-staging') {
                            ...
                            dir('angular-17-client') {
                                bat 'npm install'
                                bat 'npx ng build --configuration staging --base-href /staging/'
                            }
                            script {
...
def stopBackend(String port) {
    powershell '''
        Get-NetTCPConnection -LocalPort ''' + port + ''' -ErrorAction SilentlyContinue | ForEach-Object {
            Stop-Process -Id $_.OwningProcess -Force
        }
    '''
    println("Backend detenido en puerto " + port)
}

def runBackend(String env, String jarPath, String port) {
    bat """
        start cmd /c java ${params.JAVA_OPTS} -jar ${jarPath} --spring.profiles.active=${env} --server.port=${port} > ${env}-backend.log 2>&1
    """
    println("Backend iniciado en ${env} en puerto ${port}")
}